...
def read_urls(filename):
    ...
    text = f.read()
    r = re.search(r'([A-Za-z1-9\.-]+)_access\.log', filename)
    if not r:
        sys.exit(1)
    host = r.group(1)
    reg_ex = re.compile(r'GET\s([\/\w]+\.jpg)')

    return ['http://{}{}'.format(host, x) for x in sorted(set(reg_ex.findall(text)))]
...
def download_images(img_urls, dest_dir):
    ...
    for i, url in enumerate(img_urls):
        img_filename = os.path.join(img_dir, 'img%d.jpg' % i)
        urllib.urlretrieve(url, img_filename)
        img_filename_list.append(os.path.basename(img_filename))
    ...
    """.format(''.join(['<img src="img/{}">'.format(x) for x in img_filename_list]))
    f.write(html)
